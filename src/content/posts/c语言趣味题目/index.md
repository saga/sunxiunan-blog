---
title: C语言趣味题目
description: "http://stevenkobes.com/ctest.html 在这个网站上发现一套很有趣的C语言测试题， &hellip; \n继续阅读“C语言趣味题目”"
...
[http://stevenkobes.com/ctest.html](http://stevenkobes.com/ctest.html "http://stevenkobes.com/ctest.html")
...
[![image](./attachments/image_thumb1.png)](http://sunxiunan.com/wp-content/uploads/2010/05/image1.png)
...
关键点在于理解setjmp以及longjmp，（[http://en.wikipedia.org/wiki/Setjmp.h](http://en.wikipedia.org/wiki/Setjmp.h "http://en.wikipedia.org/wiki/Setjmp.h") ）第一次运行到setjmp，会设置jmp\_buf，然后返回0。当调用longjmp时，会把longjmp里面的非0值作为setjmp的返回值返回（如果longjmp的value参数为0，setjmp恢复后返回1，也就是当恢复到setjmp存储点的时候，setjmp一定不会返回0）。
...
j = sizeof(++i + ++i);  
printf(“i=%d j=%d\\n”, i, j);  
这道题考的内容其实就是sizeof，我在这篇文章里提到过[http://sunxiunan.com/?p=1637](http://sunxiunan.com/?p=1637 "http://sunxiunan.com/?p=1637") sizeof如果计算表达式，那么表达式是不会做计算的，也就是不管加加减减，sizeof就是针对i计算大小。在32位机器上，这个j应该为4。
...
这道题我在[http://sunxiunan.com/?p=1637](http://sunxiunan.com/?p=1637 "http://sunxiunan.com/?p=1637") 也提到过相关知识点，也就是p实际指向\*(buf + 1 + 5)，写的更诡异一些就是p=5\[buf +1\];也是同样结果。
...
这道题也是两个知识点，一个是可变参数函数定义以及如何实现，va\_arg会把5，7依次取出来。另一个知识点是i &= i-1，实际上是计算了i二进制形式中1的个数，每次计算都会消减掉最低有效位上的1。比如7二进制表示为111。i &= i –1的计算结果依次为110，100, 000 （也就是0）。在hacker’s Delights这本书里介绍了很多类似技巧。
